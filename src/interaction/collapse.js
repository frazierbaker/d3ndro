export default {
    leafClick(d, parsedJSON, spacing, height) {
		...
		$('#d3ndro').trigger('d3ndro:leaf:click', [d, parsedJSON, spacing, height]);
	},

	toggleCollapse(id, parsedJSON) {
		let shouldCollapse = !d3.select(`#node${id}`).classed('d3ndro-collapsed');
		if (shouldCollapse) {
	...
	collapse(id, parsedJSON) {
		let mainElement = d3.select(`#node${id}`);
		
        // Fire event:
        $('#d3ndro').trigger('d3ndro:collapse', [mainElement, parsedJSON]);

		mainElement.classed("d3ndro-collapsed", true);
		parsedJSON.progeny(id).map((node) => {
			let element = d3.select(`#node${node.id}`)
	...
	uncollapse(id, parsedJSON) {
        ...
        // Fire event:
        $('#d3ndro').trigger('d3ndro:uncollapse', [mainElement, parsedJSON]);
