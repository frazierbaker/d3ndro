export default {
	drawInternals(internals, parsedJSON, spacing, height, internalRadius) {
		let radius = internalRadius ? internalRadius : spacing / 4;
		...
		internalEnterGroup.append('rect')
			...
			.on('click', (d) => window.d3ndro.interaction.toggleCollapse(d.id, parsedJSON));
